...
data class RightTriangleSize(var base: MutableList<Double>, var altura: MutableList<Double>){
    fun superficie(): MutableList<Double> {
    ...
    fun perimetre(): MutableList<Double> {
        val resu = mutableListOf<Double>()
        for(i in 0..base.lastIndex){
    ...
    fun toOutput(){
        for(i in 0..base.lastIndex) {
            println("Un triangle de ${base[i]} x ${altura[i]} té ${superficie()[i]} d'àrea i ${perimetre()[i]} de perímetre.")
        }
    }
}
